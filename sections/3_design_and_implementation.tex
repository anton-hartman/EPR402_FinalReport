...
%This is also faster than the top speed of a mosquito which is 0.67~m/s.
...
\subsubsection{Laser turret}
The specific geometry of the laser turret was designed with the goal to practically obtain a sufficiently small lateral step size of the laser on the back wall of the mosquito enclosure, while maintaining sufficient speed.

The dimensions of the mirrors were chosen for practicality as $30 \times 15$~mm.
...
\begin{equation}
    \theta_{step}^{max} = \arctan\left({\frac{x_{min}}{z_{min}}}\right) = \arctan\left({\frac{\requiredLatStepSizeMM \text{mm}}{\zMinCM \text{cm}}}\right) = \stepRes\degree\,.
...
The required torque and \gls{rpm} were calculated to enable appropriate motor selection.


\paragraph{Torque}\mbox{}\\
...
The moment of inertia for the mirror was calculated by assuming the mirror is solid cylinder with diameter equal to the width of the mirror. This will produce an inflated moment of inertia, however, this is not a concern since having extra torque is not a problem. The moment of inertia $I$ of a cylinder with rotation about its central axis as seen in \autoref{fig:cylinder_central_axis} is given by
\begin{equation}
    I = \frac{1}{2}Mr^2\,,
\end{equation}
where $M$ is the mass of the cylinder and $r$ is the radius of the cylinder. The mass of the cylinder $M$ will be calculated using the density of glass $\rho = \SI{2500}{\kg\per\meter\cubed}$ and the volume of the cylinder $V$ given by
\begin{equation}
    V = \pi r^2 h\,.
\end{equation}
The height $h$ and radius $r$ of the cylinder is given by the chosen mirror dimensions as $h = 30 \text{ mm}$ and $r = 7.5 \text{ mm}$. The mass of the mirror $M$ is
\begin{equation}
    M = \rho V = \SI{2500}{\kg\per\meter\cubed} \times \pi \left(7.5 \text{ mm}\right)^2 (30 \text{ mm}) = 13.25 \text{ g}\,.
\end{equation}
Thus, the moment of inertia of the mirror is
\begin{equation}
    I = \frac{1}{2}Mr^2 = \frac{1}{2}(13.25 \text{ g})(7.5 \text{ mm})^2 = \SI{3.73e-7}{\kg\per\meter\squared}\,.
\end{equation}

The angular acceleration of the motor $\alpha$ is calculated with
\begin{equation}
...
Finally, the required torque $\tau$ is
\begin{equation}
    \tau = I\alpha = \SI{3.73e-7}{\kg\per\meter\squared} \times \SI{114}{\radian\per\second\squared} = \SI{4.35e-5}{\newton\meter}\,.
\end{equation}
...
The reflection off the front glass at the origin of the laser turret remains stationary since the laser turret itself is stationary with only the angles of the mirrors adjusted. The remaining two reflections are distinguished using the geometric properties of the hardware setup. The $yz$-plane of the hardware setup is shown in \autoref{fig:distinguish_lasers}. The laser turret is slightly below the camera which is exaggerated in \autoref{fig:distinguish_lasers} for illustration purposes. It can be seen in \autoref{fig:distinguish_lasers} that if both detections are at or below the camera origin the reflection of interest is closer to the camera origin. If both reflections are at or above the camera origin then the reflection of interest is further from the camera origin. If the reflections are on either side of the camera origin the reflection of interest is the reflection above the camera origin. This logic is easily extended to three dimensions by considering the $yz$-plane and $xz$-plane independently.
\begin{figure}[h]
    \centering
    \begin{subfigure}[t]{0.45\textwidth}
